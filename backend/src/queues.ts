  ...
  try {
    ...
    if (count > 0) {
      schedules.map(async schedule => {
        ...
        logger.info(`Envio programado para: ${schedule.contact.name}`);
      });
    }
  } catch (e: any) {
    Sentry.captureException(e);
    ...
    if (campaigns.length > 0) {
      ...
      const promises = campaigns.map(async (campaign) => {
        try {
          ...
          logger.info(
            `Campaña enviada a la cola de procesamiento: Campaña=${campaign.id}, Retraso inicial=${delay}`
          );
      ...
      logger.info('Todas las campañas han sido procesadas y agregadas a la cola..');
    }
  } catch (err) {
    Sentry.captureException(err);
    ...
    if (campaign) {
      ...
      if (isArray(contacts)) {
        ...
        for (let i = 0; i < contactData.length; i++) {
          ...
          logger.info(`Registro enviado al departamento de activación: Campanha=${campaign.id};Contato=${contacts[i].name};delay=${delay}`);
          // }
        }
...
async function handleDispatchCampaign(job) {
  try {
    ...
    logger.info(
      `Activador de campaña solicitado: Campanha=${campaignId};Registro=${campaignShippingId}`
    );
                ...
                if (!userId) {
                  ...
                  if (randomUserId !== undefined && await findUserById(randomUserId, ticket.companyId) > 0) {
                    ...
                    if (sendGreetingMessageOneQueues) {
                      ...
                      await SendWhatsAppMessage({ body: `\u200e *asistente virtual*:\nEspere mientras localizamos a un asistente.... ¡Serás atendido pronto!`, ticket: ticketToSend });
                    ...
                    //await ticket.reload();
                    logger.info(`Chat ID ${ticket.id} actualizado a UserId ${randomUserId} - ${ticket.updatedAt}`);
                  } else {
