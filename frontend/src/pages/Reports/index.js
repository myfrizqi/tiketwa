...
const Reports = () => {
  ...
  const exportarGridParaExcel = async () => {
    setLoading(true); // Establece el estado de carga en verdadero durante la carga.
    ...
    try {
      const data = await getReport({
        ...
        dateFrom,
        dateTo,
        page: 1, // Pasar el número de página a la API
        pageSize: 9999999, // Pasar el tamaño de la página a la API
      ...
      setPageNumber(pageNumber); // Actualiza el estado de la página actual.
    } catch (error) {
      toastError(error);
    } finally {
      setLoading(false); // Establece el estado de carga en falso después de la carga.
    }
  ...
  const handleFilter = async (pageNumber) => {
    ...
    try {
      const data = await getReport({
        ...
        dateFrom,
        dateTo,
        page: pageNumber, // Pasar el número de página a la API
        pageSize: pageSize, // Pasar el tamaño de la página a la API
        onlyRated: onlyRated ? "true" : "false"
      });

      setTotalTickets(data.totalTickets.total);

      // Comprueba si hay más resultados para definir hasMore
      setHasMore(data.tickets.length === pageSize);

      setTickets(data.tickets); // Si es la primera página, reemplace los CHATS.
      ...
      <div>
        <Grid container>
          <Grid item xs={12} sm={10} md={10}>
            ...
            <Pagination
              count={Math.ceil(totalTickets / pageSize)} // Calcula el número total de páginas en función del número total de tickets y el tamaño de página
              page={pageNumber} // Define a página atual
              onChange={(event, value) => handleFilter(value)} // Función de devolución de llamada para cambios de página.
            />
          </Grid>
