...
const FlowBuilder = () => {
  ...
  const handleDuplicateFlow = async flowId => {
    try {
      ...
      toast.success("Flujo duplicada exitosamente");
    } catch (err) {
      toastError(err);
    }
  };
  ...
  return (
    <MainContainer className={classes.mainContainer}>
      ...
      >
        {deletingContact
          ? `¿Estás seguro de que deseas eliminar este flujo? Se perderán todas las integraciones relacionadas.`
          : `${i18n.t("contacts.confirmationModal.importMessage")}`}
      </ConfirmationModal>
      <ConfirmationModal
        title={
          deletingContact
            ? `¿Quieres duplicar el flujo? ${deletingContact.name}?`
            : `${i18n.t("contacts.confirmationModal.importTitlte")}`
        }
      ...
      <MainHeader>
        <Title>Flujo de conversación</Title>
