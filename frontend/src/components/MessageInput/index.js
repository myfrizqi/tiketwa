...
const useStyles = makeStyles((theme) => ({
  ...
  invertedFabMenu: {
    border: "none",
    borderRadius: 50, // Establece el radio del borde en 0 para eliminar cualquier borde.
  ...
  invertedFabMenuMP: {
    border: "none",
    borderRadius: 0, // Establece el radio del borde en 0 para eliminar cualquier borde.
    boxShadow: "none", // quitar la sombra
    width: theme.spacing(4), // Ajusta el tamaño según tus preferencias.
...
const MessageInput = ({ ticketId, ticketStatus, droppedFiles, contactId, ticketChannel }) => {
  ...
  const isMobile = useMediaQuery('(max-width: 767px)'); // Ajuste el valor según sea necesario
  const [placeholderText, setPlaceHolderText] = useState("");

  // Determinar el texto del marcador de posición según el estado del ticket
  useEffect(() => {
    ...
    // Limite la longitud del texto del marcador de posición solo en entornos móviles
    const maxLength = isMobile ? 20 : Infinity; // Establecer el límite sólo en el móvil
  ...
  const handleSendLinkVideo = async () => {
    const link = `https://meet.jit.si/CRMWHATSAPP${ticketId}`;
  ...
  const getStatusSingMessageLocalstogare = () => {
    ...
    //si hay una llave "sendSingMessage"
  ...
  const handleUploadMedia = async (mediasUpload) => {
    ...
    // Asegúrese de que la variable medias esté completa antes de continuar
