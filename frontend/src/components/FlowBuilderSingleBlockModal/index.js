...
const FlowBuilderSingleBlockModal = ({
  ...
  const moveElementUp = id => {
    setElementsSeq(old => {
      ...
      if (index !== -1 && index > 0) {
        // Comprueba si el elemento se encontró en la matriz y no está en la primera posición
        const novoArray = [...array]; // Crea una copia de la matriz original.
        const elementoMovido = novoArray.splice(index, 1)[0];
        novoArray.splice(index - 1, 0, elementoMovido);
        return novoArray;
      }
  ...
  function moveItemParaFrente(posicao) {
    setElements(old => {
      ...
      return array; // Devuelve la matriz original si no es posible moverla.
    });
  }
